        ...
        public void run() {
            ...
            synchronized (object) {
               // System.out.println("线程"+Thread.currentThread().getName()+"调用了wait()");
            	...
            	object.notify();
                System.out.println("1线程"+Thread.currentThread().getName()+"获取到了锁");
            }
        }
    }
    ...
    static class Thread2 extends Thread{
        ...
        public void run() {
            ...
            synchronized (object) {
                ...
                System.out.println("2线程"+Thread.currentThread().getName()+"调用了object.notify()");
            }
            //只有等调用完notify()或者notifyAll()并退出synchronized块，释放对象锁后,其余线程才可获得锁执行。
            System.out.println("2线程"+Thread.currentThread().getName()+"释放了锁");
        }
    }
}